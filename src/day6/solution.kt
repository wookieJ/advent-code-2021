...
fun main() {
    fun checkFishNumber(fishList: MutableMap<Int, Long>, days: Int): Long {
        repeat(days) {
            val zerosNumber = fishList.getOrDefault(0, 0)
            val sevensNumber = fishList.getOrDefault(7, 0)
            for (age in 0..8) {
                fishList[age] = when (age) {
                    6 -> zerosNumber + sevensNumber
                    8 -> zerosNumber
                    else -> fishList.getOrDefault(age + 1, 0)
                }
            }
        }
