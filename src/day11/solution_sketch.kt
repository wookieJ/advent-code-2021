...
fun main() {
    fun cretePowerMap(input: String): MutableMap<Pair<Int, Int>, Int> {
        ...
        input.lines().forEachIndexed { row, line ->
            line.forEachIndexed { col, it -> heightMap[Pair(col, row)] = it.toString().toInt() }
        }
        return heightMap
    }
    ...
    fun part1(input: String): Int {
        ...
        var flashes = 0
        repeat(100) { i ->
            energies = energies.mapValues { it.value + 1 }.toMutableMap() // step 1
...
private fun Pair<Int, Int>.adjacent(rowsNumber: Int, colsNumber: Int): List<Pair<Int, Int>> {
    return listOf(
        ...
        Pair(this.first, this.second + 1),
        Pair(this.first + 1, this.second + 1),
    ).filter { it.first in 0 until colsNumber && it.second in 0 until rowsNumber }
}
